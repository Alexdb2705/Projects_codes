...
def main(args):
    ...
    if not args.background:
        ...
        NewFasant_creation(args.img, args.stl, args.rot_path, args.NFpath,
                    args.num,
                    args.NProcs, args.BW, args.nBW, args.f0, args.fBW, args.nfBW)
...
def cartesian_to_spherical(x):
    theta = (np.arccos(x[2] / np.sqrt(x[0]**2 + x[1]**2 + x[2]**2)))*180/np.pi         # Obtención del ángulo theta, y conversión de radianes a grados
                                                                                       # Obtención del ángulo phi, y conversión de radianes a grados
...
def escribir_geom_nurbs_from_stl(your_mesh, mesh_folder_path):        
    # Esta función escribe el fichero de geometría .nur para el stl
    # Escribir el formato .nur de mesh
    with open(mesh_folder_path, 'w') as archivo:
        archivo.write("1\n")
        archivo.write("1\n")
        ...
        archivo.write("F\n")
        archivo.write("4\n")
        archivo.write(str(len(your_mesh)*4)+"\n")
        ...
        for element in your_mesh:
            ...
            # se repite el último, 4 vértices
            archivo.write(f'{element[6]} {element[7]} {element[8]}\n')
        # Generate vertex references for every element
        for i in range(1, len(your_mesh) + 1):
            archivo.write(f'{(i-1)*4} {(i-1)*4 + 1} {(i-1)*4 + 2} {(i-1)*4 + 3}\n')
        archivo.close()
...
def blender_creation(directory_gen, stl_path, rot_path, num_samples):
    ...
    light.data.size = 10
    light.data.energy = 800  # Ajusta la intensidad de la luz según sea necesario
...
def NewFasant_creation(directory_gen, stl_path, rot_path, NF_folders_path,
                       num_samples,
                       nprocs, BW_grados, nBW_grados, f0, BW_frecuencia, nBW_frecuencia):
    ...
    datos_calculate_dict["line5"]= str(f0-BW_frecuencia/2.0)
    datos_calculate_dict["line6"]= str(f0+BW_frecuencia/2.0)
    ...
    #           plt.show()
            #copy output files
            shutil.copy(f'{os.path.join(NF_folders_path, "calculate_folder_PO")}\\result\\isar.png',f'{os.path.join(directory_gen, "ISAR")}\\isar_{int(element[0])}_{j}.png')
            shutil.copy(f'{os.path.join(NF_folders_path, "calculate_folder_PO")}\\result\\RcsFieldTO.out',f'{os.path.join(directory_gen, "ISAR")}\\RcsFieldTO_{int(element[0])}_{j}.txt')
